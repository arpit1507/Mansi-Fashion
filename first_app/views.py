...
#genrating pdf 
def generate_pdf(items, total_amount, customer):
    customer_name=customer.cleaned_data['Name']
    ...
    customer_info = [
        f"Customer Name: {customer.cleaned_data['Name']}",
        f"Customer Phone Number: {customer.cleaned_data['PhoneNumber']}",
...
def Billing(request):
    ...
    if request.method == 'POST':
        customer=forms.Customer(request.POST)
        if customer.is_valid():
            product = customer.save(commit=True)
            sold_on=product.sold_on
            bill=product.BillNo
        ...
        for item in selected_items:
            SoldItem.objects.create(Description=item.Description,Seller=item.Seller,Code=item.Code,MRP=item.MRP,Date=item.Date,FinalCode=item.FinalCode,SIZE=item.SIZE,sold_on=sold_on,BillNo=bill)
            item.delete()
        return  generate_pdf(selected_items, selected_total_amount,customer)
    return render(request, 'billing.html', {'items': items,'customer':customer})
...
def LoginPage(request):
    form=forms.AppUser()
    if request.method=='POST':
        form=forms.AppUser(request.POST)
        if form.is_valid() and form.data['Name']=='Arpit' and form.data["password"]=='Arpit':
...
def oldCustomer(request):
    ...
    if request.method == 'POST':
        ...
        selected_total_amount = sum(item.MRP for item in selected_items) 
        for item in selected_items:
            SoldItem.objects.create(Description=item.Description,Seller=item.Seller,Code=item.Code,MRP=item.MRP,Date=item.Date,FinalCode=item.FinalCode,SIZE=item.SIZE)
            item.delete()
    return render(request,"oldcustomer.html",{'items': items})
